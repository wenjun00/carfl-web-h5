      ...
      <i-row>
        <i-col :span="12">
          <i-form-item label="所属机构" prop="deptNames">
            <Cascader :data="depatmentData" :render-format="format" v-model="modifyModel.deptNames"></Cascader>
          </i-form-item>
        </i-col>
        <i-col :span="12">
...
export default class ModifyUser extends Vue {
  ...
  getData(data) {
    ...
    this.departmentService
      ...
      .subscribe(data => this.modifyModel.companyName = data.companyChinaname);
  }
  /**
   * 根据机构查询公司
   */
  ...
  getOwnerData() {
    ...
    this.modifyModel.deptNames = getParent(this.modifyModel.deptId).map(v => v.id)
  }
}
