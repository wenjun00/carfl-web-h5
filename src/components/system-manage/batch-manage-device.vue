...
<template>
  <section class="component batch-manage-device">
    ...
    <div style="width:100%;height:400px;border:1px solid #dddddd;overflow:auto">
      <i-table :columns="columns1" :data="userList" ref="table" @on-selection-change="currentSelect"></i-table>
    </div>
  </section>
</template>
...
<script lang="ts">
  ...
  import {
    UserDeviceService
  } from "~/services/manage-service/user-device.service";
  ...
  export default class BatchManageDevice extends Vue {
    ...
    onKeyUserDevice() {
      if (!this.multipleSelect.length) {
        ...
        let userIds = this.multipleSelect.map(v => v.id);
