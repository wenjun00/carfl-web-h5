<template>
  <section class="component data-box">
    <div class="table">
      <i-table ref="table" class="i-table" :columns="tableColumns" :data="data" stripe :highlight-row="highlightRow" @on-row-click="rowClick"
        :width="width" :height="height" @on-current-change="currentChange" @on-selection-change="currentSelect" size="small"></i-table>
    </div>
    ...
    <template class="dialog">
      <i-modal title="列配置" v-model="dialog.dataBoxConfig" @on-ok="updateConfig">
        <data-box-config ref="data-box-config" v-model="dialog.dataBoxConfig" :id="id" v-if="filterColumns" :dataColumns="columns"
          :configColumns="filterColumns" @close="dialog.dataBoxConfig=false"></data-box-config>
      </i-modal>
    </template>
  </section>
</template>
