...
<template>
  <section class="component upload-the-fodder ">
    ...
    <i-row>
      <div class="item-shuliang-div1">
        <div v-if="type!==0" class="item-shuliang-div2">
          <div @click="uploadFile">
            <Icon type="plus-circled" class="item-shuliang-icon" size="40" color="#265ea2"></Icon>
            <div>点击添加附件</div>
          </div>
          <span class="item-shuliang-span">支持jpg/pdf/png格式建议大小不超过10M</span>
        </div>
        <div class="demo-upload-list" v-for="(item,id) in fodderList" :key="id">
          <img class="item-upload-url" :src="item.url">
...
<style lang="less" scoped>
  ...
  .item-shuliang-div2 {
    height:200px;width:200px;border:1px solid #dddddd;cursor:pointer;text-align:center;
  }
  .item-shuliang-icon{
    display:block;margin-top:60px;
  }
  .item-upload-url{
    height:200px;width:200px;border:1px solid #C2C2C2;
  }
  .item-shuliang-span{
    color:gray
  }
