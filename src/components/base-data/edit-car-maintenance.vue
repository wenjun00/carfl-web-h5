...
export default class AddPeriods extends Vue {
	...
	created() {
		this.ruleValidate = {
			brandId: [{ required: true, message: '您输入的内容不能为空', trigger: 'blur' }],
			modelName: [{ required: true, message: '您输入的内容不能为空', trigger: 'blur' }],
			carColour: [{ required: true, message: '您输入的内容不能为空', trigger: 'blur' }],
			carEmissions: [{ required: true, message: '您输入的内容不能为空', trigger: 'blur' }],
		};
	}
	vaildFun() {
		...
		formVal.validate(valid => {
			...
			this.carService.updateVehicle(this.editMessage).subscribe(
				val => {
