<template>
  <section class="page reclaimed-case">
    <data-form :model="reclaimedModel" @onSearch="refreshData">
    ...
    <data-box :data="reclaimedDatSet" @onPageChange="refreshData" :page="pageService">
      <template slot="columns">
        ...
        <el-table-column prop="createDate" label="操作" min-width="90">
          <template slot-scope="scope">
            <el-button type="text" @click="checkCaseInfo(scope.row.id)">案件详情</el-button>
          </template>
        </el-table-column>
      </template>
    </data-box>
...
<script lang="ts">
  ...
  export default class ReclaimedCase extends Vue {
    @Dependencies(PageService) private pageService: PageService;
    @Dependencies(orderService) private orderService: orderService;

    // 角色列表数据集
    private reclaimedDatSet: Array < any > = [];
    // 角色数据实体
    private reclaimedModel: any = {
      name: ""
    };
     ...
     */
    refreshData() {
      this.orderService.query('REFUNDED', this.pageService, {
        trustee: "asc",
        "vehicle.licensePlateNumber": "asc"
      }).subscribe(data => {
        this.reclaimedDatSet = data;
      });
    }
  }
