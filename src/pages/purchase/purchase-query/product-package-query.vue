...
<template>
  <section class="page product-package-query">
    ...
    <data-form date-prop="timeSearch" :model="seachParams" @on-search="productSeach" :page="pageService">
      <template slot="input">
...
export default class ProductPackageQuery extends Page {
  ...
  created() {
    ...
    },
    {
      ...
      key: 'uploadTime',
      minWidth: this.$common.getColumnWidth(6),
      render: (h, { row }) => h('span', this.$filter.dateFormat(row.uploadTime, 'yyyy-MM-dd hh:mm:ss'))
    },
    {
      align: 'center',
      editable: true,
