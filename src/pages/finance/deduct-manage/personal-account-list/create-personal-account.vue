...
<template>
  ...
  <i-form :label-width="110" :model="model" ref="form">
    <i-form-item label="账户类型" prop="accountType">
      <i-select class="create-account" v-model="model.accountType">
        <i-option v-for="{value,label} in $dict.getDictData('0107')" :key="value" :label="label" :value="value"></i-option>
      </i-select>
    </i-form-item>
    <i-form-item label="客户姓名" prop="name">
      <i-input class="create-account" v-model="model.name"></i-input>
    </i-form-item>
    <i-form-item label="证件类型" prop="certificateType">
      <i-select class="create-account" v-model="model.certificateType">
        <i-option label="身份证" value="身份证" key="身份证"></i-option>
      </i-select>
    </i-form-item>
    <i-form-item label="证件号码" prop="certificateNumber">
      <i-input class="create-account" v-model="model.certificateNumber"></i-input>
    </i-form-item>
    <i-form-item label="开户银行" prop="depositBank">
      <i-input class="create-account" v-model="model.depositBank"></i-input>
    </i-form-item>
    <i-form-item label="开户省市" prop="depositCity">
      <i-select class="account-form" v-model="model.province">
        <i-option v-for="{value,label} in $city.getCityData({ level : 1 })" :key="value" :label="label" :value="value"></i-option>
      </i-select>
      <i-select class="account-form" v-model="model.depositCity">
        <i-option v-for="{value,label} in model.province ? $city.getCityData({ level: 1, id: model.province }) : []" :key="value"
          :label="label" :value="value"></i-option>
      </i-select>
    </i-form-item>
    <i-form-item label="银行卡号" prop="cardNumber">
      <i-input class="create-account" v-model="model.cardNumber"></i-input>
    </i-form-item>
    <i-form-item label="银行预留手机号" prop="reservedPhoneNumber">
      <i-input class="create-account" v-model="model.reservedPhoneNumber"></i-input>
    </i-form-item>
