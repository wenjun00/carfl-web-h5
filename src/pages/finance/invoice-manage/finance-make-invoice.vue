...
export default class FinanceMakeInvoice extends Page {
  ...
  */
  ...
  paymentDetails(row) {
    // console.log(row)
    let id = row.collectMoneyDetailId
    let ordid = row.orderId
    this.paymentDetailsModal = true
    let paymentDetailsModal = this.$refs['payment-details'] as PaymentDetails
    paymentDetailsModal.obtainDetails(id,ordid)
