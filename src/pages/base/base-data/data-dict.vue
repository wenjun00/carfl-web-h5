            ...
            <i-row>
                <i-col :span="6">
                    ...
                    <div style="width:250px;height:600px;border-left:1px solid black;border-right:1px solid black;border-bottom:1px solid black">
                        <div v-for="item in dataType" :key="item.id" :value="item.name" :class="{'dataTypeCss':checkId===item.id}" style="cursor:pointer;width:228px;height:40px;line-height:40px;font-size:16px;postion:relative;margin:auto" @click="checkDataType(item)">
...
export default class DataDict extends Page {
  ...
  addData() {
    ...
    console.log(tb, 999);
    this.dataNames.push({id:this.dataNames.length+1,name: ""})
  }
  deleteDataDict() {
    this.$Modal.confirm({
      title: "提示",
