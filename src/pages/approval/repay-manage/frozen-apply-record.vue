...
<template>
  <section class="page frozen-apply-record">
    ...
    <i-row class="data-form" v-if="searchOptions">
      <i-input class="data-form-item search-input" v-model="frozenModel.orderInfo"
               placeholder="请录入客户姓名\证件号码\订单号\手机号查询"></i-input>
      <span class="data-form-item date">日期：</span>
      <i-date-picker class="data-form-item date-picker" v-model="frozenModel.applyDateStart"
                     placeholder="起始日期"></i-date-picker>
      ~
      <i-date-picker class="data-form-item date-picker" v-model="frozenModel.applyDateEnd"
                     placeholder="终止日期"></i-date-picker>
      <i-select class="data-form-item collect" placeholder="全部结算通道" v-model="frozenModel.collectMoneyMethod"
                clearable>
    ...
    <data-box :id="348" :columns="columns1" :data="frozenList" @onPageChange="getFrozenList"
