...
<template>
  <section class="page customer-repay-query">
    ...
    <i-row class="data-form" v-if="searchOptions">
      <i-input class="data-form-item search-input" placeholder="请录入客户姓名\证件号码"
               v-model="customerRepayModel.dynamicParam"></i-input>
      <i-select class="data-form-item payment-status" placeholder="全部还款状态" v-model="customerRepayModel.paymentStatus"
                clearable>
        <i-option v-for="{value,label} in $dict.getDictData('0104')" :key="value" :label="label"
                  :value="value"></i-option>
      </i-select>
      <i-select class="data-form-item settlement-channel" placeholder="全部结算通道"
                v-model="customerRepayModel.settlementChannel"
                clearable>
