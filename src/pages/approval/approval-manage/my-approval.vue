  ...
  <section class="page my-approval">
    ...
    <i-row class="data-form" v-if="searchOptions">
      <i-input class="data-form-item search-input"
               v-model="myOrderModel.personalInfo" placeholder="请录入客户姓名\证件号码\手机号查询"></i-input>
      <span class="data-form-item date">日期：</span>
      <i-date-picker class="data-form-item date-picker" v-model="myOrderModel.startTime"
                     placeholder="起始日期"></i-date-picker>
      ~
      <i-date-picker class="data-form-item date-picker" v-model="myOrderModel.endTime"
                     placeholder="终止日期"></i-date-picker>
      <span class="data-form-item province-city">省市：</span>
      <i-select class="data-form-item select province" placeholder="选择省" v-model="myOrderModel.province" clearable>
        <i-option v-for="{value,label} in this.$city.getCityData({ level : 1 })" :key="value" :label="label"
                  :value="value"></i-option>
      </i-select>
      <i-select class="data-form-item select city" placeholder="选择市" v-model="myOrderModel.city" clearable>
        <i-option
          v-for="{value,label} in this.myOrderModel.province ? this.$city.getCityData({ level: 1, id: this.myOrderModel.province }) : []"
          :key="value" :label="label" :value="value"></i-option>
      </i-select>
      <span class="data-form-item product-type">产品类型：</span>
      <i-select class="data-form-item select product-type" placeholder="产品类型" v-model="myOrderModel.productType"
                clearable>
        <i-option v-for="{value,label} in $dict.getDictData('0419')" :key="value" :label="label"
                  :value="value"></i-option>
      </i-select>
      <i-button class="data-form-item serch-button blueButton" @click="getMyOrderList">搜索</i-button>
    </i-row>
    <data-box :id="280" :columns="columns1" :data="myOrderList" @onPageChange="getMyOrderList"
              :page="pageService"></data-box>
    <!--Modal-->
    <template class="modal-approve">
      <i-modal v-model="approveModal" title="审批" width="800" class="approve">
        <approve ref="approve"></approve>
        <div class="modal-approve-item" slot="footer">
          <i-button class="modal-approve-item tuihui" @click="backToResource">退回资源池</i-button>
          <div class="modal-approve-item left"></div>
          <i-button class="modal-approve-item left-button" @click="submitToGray">灰名单</i-button>
          <i-button class="modal-approve-item left-button" @click="submitToblack">黑名单</i-button>
          <i-button class="modal-approve-item left-button" @click="submitToInternal">提交内审</i-button>
          <div class="modal-approve-item right"></div>
          <i-button class="modal-approve-item right-button" size="large"
    ...
    <template class="modal-blacklist">
      <i-modal :title="rejectOrBlackFlag?'拒绝':'黑名单'" v-model="blackListModal">
        <i-form>
          <i-form-item v-if="rejectOrBlackFlag">
            <i-select class="modal-blacklist-item result" placeholder="请选择结果" @on-change="changeSelectOne">
              <i-option label="拒绝" :value="375" :key="375"></i-option>
            </i-select>
            <i-select class="modal-blacklist-item reason" placeholder="全部拒单原因"
            ...
            <i-select class="modal-blacklist-item detail" placeholder="全部拒单细节"
          ...
          <i-form-item>
            <i-input type="textarea" v-model="approvalRecordModel.remark" placeholder="请录入详细原因（非必填，限制1000字以内）"
                     :maxlength="1000"></i-input>
          </i-form-item>
        </i-form>
        <div slot="footer">
    ...
    <template class="modal-reback">
      <i-modal title="退回申请" v-model="rebackModal" @on-ok="approveModal=false">
        <i-form>
          <i-form-item>
            <i-select class="modal-raback-item result" placeholder="请选择结果" @on-change="changeSelectOne">
              <i-option label="退件" :value="374" :key="374"></i-option>
            </i-select>
            <i-select class="modal-raback-item reason" placeholder="全部拒单原因"
                      v-model="approvalRecordModel.second" @on-change="changeSelectTwo">
              <i-option v-for="item in refuseReason" :key="item.second" :label="item.second"
                        :value="item.second"></i-option>
            </i-select>
            <i-select class="modal-raback-item detail" placeholder="全部拒单细节"
                      v-model="approvalRecordModel.approveReasonId">
              <i-option v-for="item in refuseDetail" :key="item.id" :label="item.detail" :value="item.id"></i-option>
            </i-select>
          </i-form-item>
