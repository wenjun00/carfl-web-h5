  ...
  <section class="page internal-audit-manage">
    ...
    <i-row class="data-form" v-if="searchOptions">
      <i-input class="data-form-item search-input"
               ...
               v-model="approvalModel.personalInfo"></i-input>
      <span class="data-form-item date">日期：</span>
      <i-date-picker class="data-form-item date-picker"
                     v-model="approvalModel.startTime" placeholder="起始日期"></i-date-picker>
      ~
      <i-date-picker class="data-form-item date-picker"
                     v-model="approvalModel.endTime" placeholder="终止日期"></i-date-picker>
      <span class="data-form-item province-city">省市：</span>
      <i-select class="data-form-item select province" placeholder="选择省"
                v-model="approvalModel.province" clearable>
        <i-option v-for="{value,label} in this.$city.getCityData({ level : 1 })" :key="value" :label="label"
                  :value="value"></i-option>
      </i-select>
      <i-select class="data-form-item select city" placeholder="选择市"
                v-model="approvalModel.city" clearable>
        <i-option
      ...
      <i-select class="data-form-item select product-type" placeholder="产品类型"
                v-model="approvalModel.productType" clearable>
        <i-option v-for="{value,label} in $dict.getDictData('0419')" :key="value" :label="label"
                  :value="value"></i-option>
      </i-select>
      <i-button class="data-form-item serch-button blueButton" @click="getInternalAuditList">
        搜索
      </i-button>

    </i-row>

    <data-box :id="295" :columns="columns1" :data="internalList" @onPageChange="getInternalAuditList"
