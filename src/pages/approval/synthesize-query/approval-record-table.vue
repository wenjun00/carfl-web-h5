...
<template>
  <section class="page approval-record-table">
    <i-row class="command">
      <span class="command-item">审核记录表</span>
    ...
    <i-row class="data-form" v-if="searchOptions">
      <i-select class="data-form-item state" placeholder="全部状态" v-model="approvalRecordModel.type"
                @on-change="changeSelectOne" clearable>
      ...
      <i-select class="data-form-item type" placeholder="通过类型" v-if="passSelect"
                v-model="approvalRecordModel.second" clearable>
      ...
      <i-select class="data-form-item reason" placeholder="全部拒单原因" v-if="!passSelect"
                v-model="approvalRecordModel.second" @on-change="changeSelectTwo" clearable>
        <i-option v-for="item in refuseReason" :key="item.second" :label="item.second" :value="item.second"></i-option>
      </i-select>
      <i-select class="data-form-item detail" placeholder="全部拒单细节" v-if="!passSelect"
                v-model="approvalRecordModel.detail" clearable>
      ...
      <i-date-picker class="data-form-item date-picker"
                     v-model="approvalRecordModel.startTime"
                     placeholder="起始日期"></i-date-picker>
      ~
      <i-date-picker class="data-form-item date-picker"
                     v-model="approvalRecordModel.endTime"
                     placeholder="终止日期"></i-date-picker>
      <i-button class="data-form-item search-button blueButton" @click="getApprovaRecordList">
        搜索
      </i-button>
    </i-row>
    <data-box :id="356" :columns="columns1" :data="approvalRecordList" @onPageChange="getApprovaRecordList"
